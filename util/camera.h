...
public:
    ...
    Camera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = YAW, float pitch = PITCH) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
    {
		...
		this->view = glm::lookAt(position, // camera position in world space
			glm::vec3(0, 0, 0),			       // camera target in world space
			up);			   // camera up vector in world space (to calculate right vector)
	...
	void setView(glm::vec3& camPos, glm::vec3& camTarget, glm::vec3& camUp) {
		this->view = glm::lookAt(camPos, camTarget, camUp);
	...
	glm::vec3 &getCamUp() { return WorldUp; }
